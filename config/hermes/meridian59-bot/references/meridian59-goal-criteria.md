...
```json
{
  ...
  "constraints": {
    "avoid_death": true,
    "bank_before_hazard": true,
    "operator_notes": "Use the smallest grounded level gap and only a matchup supported by the character's verified equipped weapon and source vulnerability facts. Treat progression candidates as eligible, not proven safe. Call prey once for the unchanged state and hunting_grounds once for the selected creature; after both are grounded, launch or change tactics instead of repeating either read. Inspect the full room spawn risk. Recover health and equip_best in a sanctuary; decide whether banking helps from live financial context. Ordinary resting reaches 80 vigor. Food is optional endurance: at that rest-reachable floor the keeper consumes carried or self-created food opportunistically unless eat_before_fighting=false, while buy_food stays false unless the planner deliberately authorizes paid acquisition from current financial evidence. Reagents count only when the known-spell list verifies create food. After swarm evidence, start safe-spot background farming from that sanctuary with flee_below=0.425 (the exact 17/40 boundary), a 0.90+ hold-resume threshold, fight_above_vigor=80 unless the planner deliberately chooses a higher food-assisted floor, buy_food=false by default, break_out_via_logoff=false, and an exact non-quarantined assigned room. Let the keeper own hazardous travel and combat; do not travel into the monster room first or issue foreground actions while it runs. A safe_spot.works label alone is not proof, but ordinary retaliation, withdrawal, or death from the monster already engaged or pulled to the wall does not disprove it. Retire the exact coordinate only when live evidence shows a new, previously unengaged monster can acquire the character there, or an explicit placement/geometry failure is reported. Successful withdraw/rest/resume cycles remain productive farm work however often they repeat, and verified kills renew the farm phase's elapsed-time lease. Stop at the phase target and rerank prey when max HP reaches the prey's level."
  },
  "priority": 90,
