...
| AT-LLM-029 | P0 | Give the planner several source-verified safe-ending candidates and a complete persona. It chooses one and returns a final exact-room `travel` step. Reject plans that omit `safe_ending`, select an unverified/unsafe room, bind a non-final or non-travel step, or later travel to a different room. After an unsafe-location criterion is briefly verified, retain the latched outcome while returning and set `succeeded` only after fresh observation verifies the selected safe room. | FR-GOAL-020, FR-CONV-002 |
