...
| AT-LLM-032 | P0 | While a keeper reports combat, travel, retreat, or recovery, observe one controller turn. The adapter calls only broker-local autopilot status plus cached look before the controller yields; current look room/vitals override stale status, slow context is marked cached/unknown, and no fresh status/inventory/spell request enters the shared packet pacer. Once keeper activity is non-urgent, the next observation performs the ordinary full refresh. | FR-PLAY-003, FR-PLAY-020; NFR-PERF-003 |
| AT-LLM-033 | P0 | Supply one durable `last_death` while the keeper has already changed from `farm` to `survive`, repeat it across turns and restart, and verify exactly one `character.died` event and combat-memory death. Repeat with a fresh Underworld/max-HP-loss transition while the old receipt is still present, then a late new receipt: the raw death is counted once and the late receipt only corroborates it. No case creates a safe-spot quarantine from ordinary retaliation/death alone. | FR-PLAY-006, FR-PLAY-021; NFR-REL-001 |
| AT-LLM-034 | P0 | Exhaust bounded repair with malformed tactical protocol, response-format, prompt-budget, and invalid execution-plan failures. The first failure opens a durable circuit; unchanged subsequent turns and a controller restart issue no model call. Change material inventory or the planner operation and verify exactly one bounded retry becomes eligible. | FR-PLAY-010, FR-PLAY-022; NFR-PERF-003, NFR-REL-001 |
