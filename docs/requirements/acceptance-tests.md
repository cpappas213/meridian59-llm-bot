...
| AT-LLM-026 | P0 | Pause a goal requiring max HP >= 33, then supply a fresh observation with max HP 34 and all remaining criteria true. The controller marks it succeeded without resuming it or invoking the LLM; an unmet paused goal remains paused. | FR-GOAL-018; NFR-REL-001 |
...
| AT-LLM-028 | P0 | Complete a controller-owned purchase goal that has no location/coordinate finish criteria. The controller performs no post-acquisition travel. Repeat with a non-Tos location and square explicitly present in the approved goal; it travels and positions at those exact values without substituting a default inn. | FR-GOAL-008, FR-GOAL-019 |
