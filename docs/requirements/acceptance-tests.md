...
| AT-LLM-019 | P0 | Activate a bounded max-HP goal with a validated `hunt`, `assigned_room`, and survival recipe. With verified equipment in Tos Inn, the controller launches the goal-owned farm keeper directly without repeating `prey` or `hunting_grounds`; optional banking is not a launch prerequisite, while survival preflight and quarantine rules still apply. | FR-PLAY-008, FR-PLAY-009, FR-GOAL-002 |
